...
def create_account(new_acc: NewAccount, db: Session = Depends(get_db)) -> None:
    create(db, new_acc)
...
def get_balance(
    ...
    return balance(db, account_id)


@router.put("/{account_id}/block", status_code=status.HTTP_200_OK)
def block_account(account_id: str, db: Session = Depends(get_db)) -> None:
    block(db, account_id)
...
def get_bank_statement(
    ...
    return bank_statement(account_id, db, start_date, end_date)
